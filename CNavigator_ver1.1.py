...
After the user answers correctly or the user wants to give up, ask them to input 'cccc'.
"""
...
# Teaching cycle in a lecture
def teaching_cycle(knowledge_point: str, knowledge_point_prompt: str, tutor: TeachingAgent, quiz: TeachingAgent, client: OpenAI, log_dir: str,retriever=None):
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    ...
    # The student can keep talking with tutor before he inputs cccc
    while True:
    ...
    #The student can keep trying the quiz, and he can skip it by entring cccc
    while True:
    ...
    # Lecture: {lecture}
    # Quiz: {quiz_content}
    # Student Answers: {student_answer}
...
# Teaching cycle for whole course
def main():
    ...
    try:
        ...
        with open(main_log, 'a', encoding='utf-8') as f:
            ...
            #     chapter_quiz_content = chapter_quiz.get_response(f"Create a comprehensive chapter quiz based on: {json.dumps(chapter_content)}")
